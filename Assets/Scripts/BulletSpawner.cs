...
public class BulletSpawner : MonoBehaviour
{
    ...
    void Update()
    {
        switch(type){
            ...
            if(cnt%(interval*2)==0)bulletManager.GetComponent<BulletManager>().BulletAppear(this.gameObject.transform.position,6,5,0,2,1,0);
            if(cnt%(interval*2)==interval)bulletManager.GetComponent<BulletManager>().BulletAppear(this.gameObject.transform.position,6,5,0,2,2,0);
            ...
            if(cnt%interval==0){
                bulletManager.GetComponent<BulletManager>().BulletAppear(this.gameObject.transform.position,7,1,0,3,0,Mathf.Atan2((28f*boxX-boxY-boxX*vertialZ)/50,(boxY-boxX)/5));
            }
            break;
        }
        cnt++;
